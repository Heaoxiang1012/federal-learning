...
def load_graph(path):
    ...
    for index in set(G.edges):
        jaccard = len(set(G.neighbors(index[0])) & set(G.neighbors(index[1]))) / (
                len(set(G.neighbors(index[0])) | set(G.neighbors(index[1]))) + 1)
...
if __name__ == "__main__":
    G = load_graph("../../data/genuine/celegansneural.txt")
